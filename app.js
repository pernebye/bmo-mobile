...
// ВРЕМЕННАЯ диагностика низа экрана — удалить вместе с .dbg в styles.css и index.html
(() => {
  const box = document.getElementById('dbg');
  if (!box) return;
  const probeSafe = () => {
    const p = document.createElement('div');
    p.style.cssText = 'position:fixed;left:0;bottom:0;width:1px;height:env(safe-area-inset-bottom,0px);pointer-events:none;opacity:0';
  ...
  const tick = () => {
    ...
    const open = (sheet && !sheet.hidden) ? sheet : (note && note.classList.contains('open') ? note : null);
    const L = [];
    L.push(`innerH ${innerHeight}  clientH ${document.documentElement.clientHeight}  screenH ${screen.height}`);
    L.push(`vv.h ${vv ? Math.round(vv.height) : '-'}  vv.top ${vv ? Math.round(vv.offsetTop) : '-'}  dpr ${devicePixelRatio}`);
    L.push(`ENV safe-bottom = ${probeSafe()}px   safe-top = ${getComputedStyle(document.documentElement).getPropertyValue('--safe-top').trim()}`);
    if (open) {
      const r = open.getBoundingClientRect();
      L.push(`${open.id}: top ${Math.round(r.top)} bot ${Math.round(r.bottom)} h ${Math.round(r.height)}`);
      const sc = open.querySelector('.sheet-body, .note-page-body');
      if (sc) {
        const sr = sc.getBoundingClientRect();
        L.push(`scroll: bot ${Math.round(sr.bottom)} padB ${getComputedStyle(sc).paddingBottom}`);
        L.push(`scrollH ${sc.scrollHeight} clientH ${sc.clientHeight} top ${Math.round(sc.scrollTop)}`);
        const last = sc.lastElementChild;
        if (last) L.push(`last el bot ${Math.round(last.getBoundingClientRect().bottom)}`);
      }
    } else {
      L.push('overlay: нет — открой задачу или заметку');
    }
